...
<head>
    ...
    <title id="page-title">@yield('title', 'Dashboard') — SIAKAD Guru</title>
...
<aside id="sidebar" class="flex flex-col"
    ...
    <div class="flex items-center gap-3 px-4 py-5 border-b border-white/10 shrink-0">
        ...
        <div class="brand-name overflow-hidden">
            <p class="text-white font-extrabold text-sm leading-tight">Portal Guru</p>
    ...
    <nav id="sidebar-nav" class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <a href="{{ route('guru.jadwal-ujian.index') }}"
           class="nav-item {{ request()->routeIs('guru.jadwal-ujian.*') ? 'active' : '' }}">
            ...
            <span class="sidebar-label">Jadwal Ujian Saya</span>
        </a>

        <a href="{{ route('guru.jawaban-ujian.index') }}"
           class="nav-item {{ request()->routeIs('guru.jawaban-ujian.*') ? 'active' : '' }}">
            <span class="icon">📋</span>
            <span class="sidebar-label">Jawaban Ujian Siswa</span>
        </a>
    ...
    <div class="px-4 py-4 border-t border-white/10 shrink-0 space-y-3">
        <div class="flex items-center gap-3">
            ...
            <div class="brand-name overflow-hidden">
                ...
                <p class="text-white/40 text-xs truncate">Guru</p>
            </div>
        </div>
...
function toggleSidebar() {
    if (isMobile()) {
        ...
        localStorage.setItem('guruSidebarCollapsed', sidebar.classList.contains('collapsed'));
    }
}
...
if (!isMobile() && localStorage.getItem('guruSidebarCollapsed') === 'true') {
