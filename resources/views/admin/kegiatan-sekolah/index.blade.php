...
@section('breadcrumb', 'Kegiatan Sekolah')
...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-xl font-extrabold text-slate-800">Kegiatan Sekolah</h1>
            <p class="text-sm text-slate-500">Total: <strong>{{ $kegiatan->total() }}</strong> kegiatan</p>
        </div>
        ...
        <form method="GET" class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex-1 min-w-40">
                ...
                <select name="kategori"
                    ...
                    @foreach(App\Models\KegiatanSekolah::$kategori as $val => $label)
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        @if($kegiatan->isEmpty())
            <div class="p-12 text-center">
                ...
                <h3 class="font-bold text-slate-600 mb-1">Belum ada data kegiatan</h3>
                <p class="text-sm text-slate-400">Tambahkan kegiatan baru dengan tombol di atas.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 text-left">
                            ...
                            <th class="px-4 py-3.5 font-semibold text-slate-500">Kategori</th>
                            <th class="px-4 py-3.5 font-semibold text-slate-500">Tanggal</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            <td class="px-4 py-3 text-slate-400">{{ $kegiatan->firstItem() + $i }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-slate-500 whitespace-nowrap">{{ $item->tanggal_kegiatan }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-2">
                                    <button type="button"
                                            data-id="{{ $item->id }}"
                                            data-kegiatan="{{ json_encode($item->only(['judul','kategori','tanggal_kegiatan','deskripsi','urutan','aktif','gambar']), JSON_HEX_QUOT | JSON_HEX_TAG | JSON_UNESCAPED_UNICODE) }}"
                                            data-gambar-url="{{ $item->gambarUrl() }}"
                                            data-has-gambar="{{ $item->gambar ? '1' : '0' }}"
                                            onclick="openEditModal(this.dataset.id, JSON.parse(this.dataset.kegiatan), this.dataset.gambarUrl, this.dataset.hasGambar)"
                                            class="text-xs px-3 py-1.5 rounded-lg font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 transition">
                                        Edit
                                    </button>
                                    <form method="POST" action="{{ route('admin.kegiatan-sekolah.toggle-aktif', $item) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                    ...
                                    <button type="button"
                                            onclick="confirmDeleteModal('{{ route('admin.kegiatan-sekolah.destroy', $item) }}', '{{ addslashes($item->judul) }}')"
...
<div id="modal-create"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 class="text-base font-extrabold text-slate-800">Tambah Kegiatan Sekolah</h2>
        ...
        @endif
        <form method="POST" action="{{ route('admin.kegiatan-sekolah.store') }}"
              enctype="multipart/form-data" class="px-6 py-5 space-y-4">
            @csrf
            <input type="hidden" name="_form" value="create">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Judul Kegiatan <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Tanggal / Periode <span class="text-red-500">*</span></label>
                    <input type="text" name="tanggal_kegiatan" value="{{ old('tanggal_kegiatan') }}"
                           placeholder="Contoh: 2 Mei 2025 atau 24–26 Juni 2025"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-300">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Status</label>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" placeholder="Deskripsi singkat kegiatan..."
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 mt-1">Kosongkan untuk menggunakan gambar placeholder otomatis per kategori.</p>
            </div>
...
<div id="modal-edit"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 class="text-base font-extrabold text-slate-800">Edit Kegiatan Sekolah</h2>
        ...
        <form id="form-edit" method="POST" action="" enctype="multipart/form-data" class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Tanggal / Periode <span class="text-red-500">*</span></label>
                    <input type="text" id="edit-tanggal" name="tanggal_kegiatan"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-300">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Status</label>
...
<script>
(function () {
    const editRoutes = @json(collect($kegiatan->items())->mapWithKeys(fn($item) => [$item->id => route('admin.kegiatan-sekolah.update', $item)]));
