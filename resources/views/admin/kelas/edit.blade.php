...
<div class="max-w-2xl space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-extrabold text-slate-800">Edit Kelas</h1>
            <p class="text-sm text-slate-500">{{ $kelas->nama }} — {{ $kelas->tahunAjaranLabel() }}</p>
        </div>
    </div>
            ...
            <div>
                ...
                <select name="tingkat"
                    ...
                    @foreach(\App\Models\Kelas::$tingkat as $val => $label)
                        <option value="{{ $val }}" {{ old('tingkat', $kelas->tingkat) === $val ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="jurusan"
                    ...
                    @foreach(\App\Models\Kelas::$jurusan as $val => $label)
                        <option value="{{ $val }}" {{ old('jurusan', $kelas->jurusan) === $val ? 'selected' : '' }}>
                            {{ $val }} — {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="keterangan" rows="3"
                          ...
                          class="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm resize-none
                                 focus:outline-none focus:ring-2 focus:ring-teal-300">{{ old('keterangan', $kelas->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
