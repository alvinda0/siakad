...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-xl font-extrabold text-slate-800">Ekstrakurikuler</h1>
            <p class="text-sm text-slate-500">Total: <strong>{{ $ekskul->total() }}</strong> ekstrakurikuler</p>
        </div>
        ...
        <form method="GET" class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex-1 min-w-36">
                ...
                <select name="jenis"
                    ...
                    @foreach(App\Models\Ekstrakurikuler::$jenis as $val => $label)
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        @if($ekskul->isEmpty())
            <div class="p-12 text-center">
                ...
                <h3 class="font-bold text-slate-600 mb-1">Belum ada data ekstrakurikuler</h3>
                <p class="text-sm text-slate-400">Tambahkan ekstrakurikuler baru dengan tombol di atas.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 text-left">
                            ...
                            <th class="px-4 py-3.5 font-semibold text-slate-500">Jenis</th>
                            <th class="px-4 py-3.5 font-semibold text-slate-500 hidden md:table-cell">Pembina</th>
                            <th class="px-4 py-3.5 font-semibold text-slate-500 hidden lg:table-cell">Jadwal</th>
                            <th class="px-4 py-3.5 font-semibold text-slate-500 text-center">Anggota</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            <td class="px-4 py-3 text-slate-400">{{ $ekskul->firstItem() + $i }}</td>
                            <td class="px-4 py-3">
                                @if($item->gambarUrl())
                            ...
                            <td class="px-4 py-3">
                                @if($item->jenis === 'Wajib')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold bg-red-50 text-red-700">Wajib</span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold bg-teal-50 text-teal-700">Pilihan</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-slate-500 hidden md:table-cell">{{ $item->pembina ?: '-' }}</td>
                            <td class="px-4 py-3 text-slate-500 hidden lg:table-cell whitespace-nowrap">{{ $item->jadwal ?: '-' }}</td>
                            <td class="px-4 py-3 text-center text-slate-600">{{ $item->jumlah_anggota }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-2">
                                    <button type="button"
                                            data-id="{{ $item->id }}"
                                            data-ekskul="{{ json_encode($item->only(['nama','jenis','jumlah_anggota','pembina','deskripsi','jadwal','urutan','aktif','gambar']), JSON_HEX_QUOT | JSON_HEX_TAG | JSON_UNESCAPED_UNICODE) }}"
                                            data-gambar-url="{{ $item->gambarUrl() ?? '' }}"
                                            data-has-gambar="{{ $item->gambar ? '1' : '0' }}"
                                            onclick="openEditModal(this.dataset.id, JSON.parse(this.dataset.ekskul), this.dataset.gambarUrl, this.dataset.hasGambar)"
                                            class="text-xs px-3 py-1.5 rounded-lg font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 transition">
                                        Edit
                                    </button>
                                    <form method="POST" action="{{ route('admin.ekstrakurikuler.toggle-aktif', $item) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                    ...
                                    <button type="button"
                                            onclick="confirmDeleteModal('{{ route('admin.ekstrakurikuler.destroy', $item) }}', '{{ addslashes($item->nama) }}')"
...
<div id="modal-create"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 class="text-base font-extrabold text-slate-800">Tambah Ekstrakurikuler</h2>
        ...
        <form method="POST" action="{{ route('admin.ekstrakurikuler.store') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Ekstrakurikuler <span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" placeholder="Deskripsi singkat ekstrakurikuler..."
...
<div id="modal-edit"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 class="text-base font-extrabold text-slate-800">Edit Ekstrakurikuler</h2>
        ...
        <form id="form-edit" method="POST" action="" enctype="multipart/form-data" class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Pembina</label>
                    <input type="text" id="edit-pembina" name="pembina"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-300">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Jadwal</label>
                    <input type="text" id="edit-jadwal" name="jadwal"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-300">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Jumlah Anggota</label>
                    <input type="number" id="edit-jumlah-anggota" name="jumlah_anggota" min="0"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-300">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Status</label>
...
<script>
(function () {
    const editRoutes = @json(collect($ekskul->items())->mapWithKeys(fn($item) => [$item->id => route('admin.ekstrakurikuler.update', $item)]));
    ...
    window.openEditModal = function (id, data, gambarUrl, hasGambar) {
        ...
        if (data) {
            ...
            document.getElementById('edit-pembina').value         = data.pembina         ?? '';
            document.getElementById('edit-jadwal').value          = data.jadwal          ?? '';
            document.getElementById('edit-jumlah-anggota').value  = data.jumlah_anggota  ?? 0;
    ...
    @php
        ...
        if (old('_form') === 'edit' && old('edit_id')) {
            $editItem = [
                ...
                'pembina'        => old('pembina'),
                'jadwal'         => old('jadwal'),
                'jumlah_anggota' => old('jumlah_anggota', 0),
                'aktif'          => old('aktif', '1') === '1',
                'urutan'         => old('urutan', 0),
