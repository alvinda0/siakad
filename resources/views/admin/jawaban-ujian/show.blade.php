...
@section('title', 'Jawaban Ujian — '.$jadwalUjian->nama)
...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                <h2 class="font-bold text-sm text-slate-700">Siswa yang Mengerjakan</h2>
                <span class="text-xs text-slate-400">{{ $sesiList->count() }} siswa</span>
            </div>
            @if($sesiList->isEmpty())
            <div class="p-8 text-center text-sm text-slate-400">Belum ada siswa yang mengerjakan.</div>
            @else
            <div class="divide-y divide-slate-100 max-h-[500px] overflow-y-auto">
                ...
                @endphp
                <a href="{{ route('admin.jawaban-ujian.show',$jadwalUjian) }}?murid_id={{ $sesi->murid_id }}"
                   class="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition {{ $isActive ? 'bg-teal-50 border-l-4 border-teal-500' : '' }}">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 mt-0.5">
                            <span class="text-[.65rem] font-bold border rounded-full px-1.5 py-0.5 {{ $badgeCls }}">{{ App\Models\SesiUjian::$status[$sesi->status]??$sesi->status }}</span>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
                ...
                <p class="text-sm text-slate-400">Klik nama siswa di sebelah kiri untuk melihat &amp; mengedit jawabannya.</p>
            </div>
            @else

            {{-- Card info siswa + tombol edit --}}
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                ...
                {{-- Tombol edit & simpan langsung di card --}}
                @if($sesiMurid)
                <div class="mt-3 pt-3 border-t border-slate-100 flex flex-wrap items-center gap-2">
                    <p class="text-xs text-slate-400 flex-1">Koreksi jawaban PG &amp; nilai essay di bawah tiap soal.</p>
                    <a href="#form-nilai"
            ...
            {{-- Form soal (edit jawaban PG + nilai essay) --}}
            @if($soalList->isNotEmpty())
            <form id="form-nilai" method="POST" action="{{ route('admin.jawaban-ujian.update-nilai',$jadwalUjian) }}">
                @csrf
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-4">
                    ...
                    <div class="flex items-center gap-3 px-5 py-3 border-b border-slate-100 bg-slate-50">
                        ...
                        <button type="button"
                                ...
                                class="ml-1 p-1.5 rounded-lg text-amber-500 hover:bg-amber-50 transition shrink-0" title="Edit kunci jawaban">
                            🔑
                        </button>
                    </div>
                    ...
                    <div class="px-5 py-4 space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            ...
                            <div class="pilihan-item {{ $cls }} flex items-center gap-2">
                                <span class="font-extrabold shrink-0">{{ $huruf }}.</span><span>{{ $teks }}</span>
                        ...
                        <div class="flex items-center gap-3 pt-2 border-t border-slate-100">
                            ...
                            <div class="flex gap-1.5">
                                ...
                                @php $isSel=$jawaban&&strtoupper($jawaban->jawaban_pg??'')===$opt; $isK=$soal->kunci_jawaban===$opt; @endphp
                                <label class="cursor-pointer">
                                    <input type="radio" name="jawaban_pg[{{ $soal->id }}]" value="{{ $opt }}" class="peer sr-only" {{ $isSel?'checked':'' }}>
                                    <span class="inline-flex items-center justify-center w-9 h-9 rounded-lg border-2 font-bold text-sm transition
                                        {{ $isK?'border-emerald-300 text-emerald-700':'border-slate-200 text-slate-500' }}
                                        peer-checked:border-teal-500 peer-checked:bg-teal-50 peer-checked:text-teal-700 hover:border-slate-300">{{ $opt }}</span>
                                </label>
                                @endforeach
                                <label class="cursor-pointer" title="Kosongkan">
                                    <input type="radio" name="jawaban_pg[{{ $soal->id }}]" value="" class="peer sr-only" {{ (!$jawaban||!$jawaban->jawaban_pg)?'checked':'' }}>
                                    <span class="inline-flex items-center justify-center w-9 h-9 rounded-lg border-2 font-bold text-xs transition border-slate-200 text-slate-400 peer-checked:border-red-300 peer-checked:bg-red-50 peer-checked:text-red-500 hover:border-slate-300">—</span>
                                </label>
                            </div>
                            <span class="text-xs text-slate-400">Kunci: <strong class="text-emerald-600">{{ $soal->kunci_jawaban??'?' }}</strong></span>
                        </div>
                        @endif
                        ...
                        <div class="flex items-center gap-3 pt-1 border-t border-slate-100">
                            ...
                            <input type="number" name="nilai_essay[{{ $soal->id }}]"
                                   value="{{ $jawaban?->nilai_essay??0 }}" min="0" max="{{ $soal->poin }}" step="1"
                ...
                <div class="sticky bottom-4 flex justify-end">
                    <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold text-white shadow-lg hover:brightness-110 transition bg-emerald-600">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
    ...
    <div id="modal-kunci" class="modal-overlay hidden" onclick="closeKunciModal(event)">
        <div class="modal-box" onclick="event.stopPropagation()">
            ...
            <p class="text-xs text-slate-500 mb-4">Mengubah kunci akan <strong class="text-slate-700">menghitung ulang nilai semua siswa</strong> yang sudah selesai.</p>
            ...
            <form id="form-kunci-pg" method="POST" class="hidden">
                ...
                <div class="grid grid-cols-4 gap-2 mb-5">
                    ...
                    <label class="cursor-pointer">
                        ...
                        <div class="text-center py-2.5 rounded-xl border-2 border-slate-200 font-bold text-slate-500 peer-checked:border-teal-500 peer-checked:bg-teal-50 peer-checked:text-teal-700 hover:border-slate-300 transition text-sm">{{ $h }}</div>
                    </label>
                    @endforeach
                </div>
...
function openKunciModal(soalId, tipe, kunciPg, kunciEssay, muridId) {
    ...
    const base = '{{ route("admin.jawaban-ujian.update-kunci", [$jadwalUjian, "__SOAL__"]) }}'.replace('__SOAL__', soalId);
    const url  = base + '?murid_id=' + muridId;
    ...
    if (tipe === 'pilihan_ganda') {
        ...
        formPg.querySelectorAll('input[type=radio]').forEach(r => r.checked = r.value === kunciPg.toUpperCase());
    } else {
        formPg.classList.add('hidden');
        formEssay.classList.remove('hidden');
        formEssay.action = url;
